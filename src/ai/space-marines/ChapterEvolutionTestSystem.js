...
export function evaluateChapterEvolutionRun(chapter, metrics = {}) {
  ...
  const checks = Object.freeze({
    ...
    vehiclesUsed: clamp01(metrics.vehicleUtilization) >= 0.75,
    wargearAccurate: clamp01(metrics.appropriateWargear) >= 0.8,
    formationsAccurate: clamp01(metrics.formationAccuracy) >= 0.75,
    commandersDeployed: clamp01(metrics.commanderPresence) >= 1,
    fieldPresence: clamp01(metrics.troopsOutsideHeadquarters) >= 0.75,
    formationReliable: clamp01(metrics.formationStuckRatio) < 0.02,
    transportsUsed: !metrics.transportOpportunities || clamp01(metrics.transportUtilization) >= 0.7,
    forwardLogistics: !metrics.frontlineDistant || Math.max(0, Number(metrics.forwardOutposts) || 0) >= 1,
    deploymentAccurate: clamp01(metrics.deploymentAccuracy) >= 0.8,
    tacticalInnovation: clamp01(metrics.tacticalInnovation) >= 0.5,
