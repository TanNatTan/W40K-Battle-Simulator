...
- Supply connectivity is revision-cached. Headquarters, warehouses, race-specific producers, friendly economic landmarks, and authored routes form the graph; production and AI queries reuse connection snapshots until topology changes.
...
- Economy output comes from race-specific production buildings, never natural resource polygons. Each producer has a fixed one-to-four-resource bundle, separate local inputs and output buffer, condition and supply-throughput modifiers, diminishing returns, and a disconnected grace period. Authored economic landmarks and trade routes remain strategic map assets.
- Legacy maps still load resource zones through a one-way migrator. Existing owned extractors retain health, construction progress, and inventory while receiving a production definition; neutral or extractor-less zones produce author-review diagnostics. New maps and saves never write resource zones, and the editor no longer exposes zone authoring.
- Completed headquarters replace lost builders and produce faction-named supply carriers. Carriers physically collect buffered output from production buildings or economic landmarks and deliver it to warehouses or headquarters. Assignments persist through pickup, loading, delivery, unloading, and return cycles; source count, distance, and output determine workforce demand.
- Economy Security prevents non-aggressive armies from abandoning threatened production. Commander AI measures base threat, defense ratio, producer guard coverage, route risk, shortages, and reserve strength; dedicated Guard Economy squads protect critical producer clusters before optional attacks or expansion resume.
- Ranged units carry exactly four times their prior total ammunition baseline while keeping weapon-specific magazine and reload timings. Combat atomically consumes both magazine and total ammunition, begins reload on the final magazine shot, and retreats for ammunition only when total ammunition reaches zero.
